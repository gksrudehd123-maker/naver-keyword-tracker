...
- [ ] 네이버 검색 API Client ID / Secret 입력 폼
- [ ] 스마트스토어 이름 입력
- [ ] API 키 유효성 테스트 버튼 (실제 호출로 확인)
...
- [ ] 네이버 쇼핑 검색 API 호출 (display=100)
- [ ] 결과에서 내 스토어명 + 상품명 매칭 → 순위 산출
- [ ] 순위 결과 날짜별 저장
- [ ] OMS의 `shopping-search.ts` 로직 참고

### Step 5 — 팝업 UI (popup.tsx)

- [ ] 상품별 키워드 순위 카드 목록
- [ ] 순위 등락 표시 (↑↓-)
- [ ] "순위 조회" 버튼
- [ ] 하단 네비게이션 (순위 조회 / 상품 관리 / 설정)

### Step 6 — 순위 추이 차트 (tabs/dashboard.tsx)

- [ ] 탭 페이지로 일별 순위 추이 차트 (Recharts)
- [ ] 키워드별 라인 차트
- [ ] 기간 필터 (7일 / 30일)
